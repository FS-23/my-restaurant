...
function Menu(){
    ...
    function handleOnClick(id){
          ...
          setTotalPrice( totalPrice => totalPrice+plat.price)
        ...
        return (
            <div className=" col-1 col-sm-2 text-white col-lg-4 mt-4 position-relative plat-container" >
                <div className="bg-dark p-2 rounded">
                    ...
                    <button className="btn btn-danger mt-2" onClick={()=> {}}>Annuler</button>
                </div>
            </div>
        )
    }
           ...
           <div className="card w-100 m-auto border-0 mt-5">
                 <div className="card-header bg-dark text-white">Mes commandes</div>
                  <div className="card-body border row m-0">
                    {
                       orders.length == 0 ? <div class="text-center">Aucune commande enregistréé</div> : orders.map((menu , index) =>  <CommandeComponent key={index} plat={menu} />)
                    }
                  </div> 
                  <div className="modal-footer border bg-light ">
