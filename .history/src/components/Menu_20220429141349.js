...
function Menu(){
    ...
    return (
        <div className="row">
           {
               menuList.map((menu , index) =>   < Plat key={index} name= {menu.name} price={menu.price} ingredient={menu.ingredient} onChoose={handleOnClick} />)
           }
           ...
           <div className="bg-dark rounded mt-5">
                    {
                        menuList.map((menu , index) =>   < Plat key={index} name= {menu.name} price={menu.price} ingredient={menu.ingredient} />)
                    }
           </div>
